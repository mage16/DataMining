...
public class AlgoAprioriClose {
	...
	public Itemsets runAlgorithm(DataSet pm25, double minsup, String input, String output) throws IOException {
		...
		System.out.println("Starting the Apriori Close Algorithm!!!");
...
//				saveItemsetToFile(entry.getKey(), entry.getValue());
	 ...
	 * @throws IOException exception if error writing output file
	 */
	private void checkIfItemsetsK_1AreClosed(List<String> levelKm1,
			List<Itemset> levelK, Map<String, Integer> mapItemCount) throws IOException {
		// for each itemset of size k-1
		for (String itemset : levelKm1) {
			boolean isClosed = true;
...
private int sameAs(Itemset itemset, String [] candidate, int posRemoved) {
	 ...
	 void saveItemset(String item, Integer support) throws IOException {
		itemsetCount++;
